...
class ProofTreeBuilder:
    @classmethod
    def run_proof_trees(
        cls, theorem_str: str, unit: ReplCompilationUnit, env: LeanProcess
    ) -> LeanTheorem:
        ...
        thm_with_sorries = replace_with_sorries(
            theorem_str, [block.span for block in theorem.by_blocks]
        )
    ...
    @classmethod
    def run_proof_tree(
        cls, src_tree: SingletonProofTree, init_proof_branch: LeanProofBranch
    ) -> ProofTree:
        ...
        while len(to_prove) > 0:
            ...
            if expansion_idx == 0 or re.match(r"^case'? ", tactic_info.tactic_string):
                ...
                goal is not None
                and goal.tag is not None
                and not any(
                    n.goal is None or n.goal.tag == goal.tag for n in src_nodes[:expansion_idx]
                )
            ):
                ...
                raise AssertionError(
                    "Applying tactic to a non-main unnamed goal is not yet supported."
                )
            ...
            src_all_children = (
                expansion_node.tactic.goals_after
                + expansion_node.tactic.spawned_goals
                + src_siblings
            )
            ...
            for sub_branch in sub_branches:
                ...
                assert len(src_all_children) >= branch_size, (
                    "Not enough singleton nodes to use in the proof."
                )
                src_children, src_all_children = (
                    src_all_children[:branch_size],
                    src_all_children[branch_size:],
                )
